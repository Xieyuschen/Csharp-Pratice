...
namespace DataAnalisis
{
   class Everyone
   {
       static void Main(string[] args)
       {
           ...
           FileStream infile = new FileStream(@"E:\C_Cpp_and_Csharp\DataAnalisis\fullevents.xlsx", FileMode.Open, FileAccess.Read);
           XSSFWorkbook inworkbook = new XSSFWorkbook(infile);
           ISheet insheet = inworkbook.GetSheet("fullevents");
           ...
           for (int i = 1; i < 60000; i++)
           {
               ...
               if (row != null)
               {
                   var player = row.GetCell(2).ToString();
                   var oper = row.GetCell(6).ToString();

                   for (int r = 0; r < 2; r++)
                   {
                       if (player.Substring(0, 7) == "Huskies")
                       {
                           var key = player +"#"+ oper;
           ...
           foreach (var item in dic)
           {
               ...
               if (item.Key[10] == '#')
               {
                   outxSSFCells[0].SetCellValue(item.Key.Substring(0,10));
                   outxSSFCells[1].SetCellValue(item.Key.Substring(11));
           ...
           FileStream outfile = new FileStream(@"E:\C_Cpp_and_Csharp\DataAnalisis\DataAnalisis\excel\Haskies队员所做动作.xlsx", FileMode.Create);
